...
class AuditCandidateAmountConsistencyCommand extends Command
{
        ...
        */
        ...
        if ($onlyChecked) {
            $candidates = $candidates
                ->filter(fn (ProductIdentificationCandidate $candidate): bool => $this->candidateAmountChecked($candidate));
        }

        if ($onlyMismatch) {
            $candidates = $candidates
                ->filter(fn (ProductIdentificationCandidate $candidate): bool => $this->candidateAmountMismatch($candidate));
        }
        ...
        $this->table([
            ...
            ->map(fn (ProductIdentificationCandidate $candidate): array => $this->candidateToTableRow($candidate))
            ->all());
     ...
     */
    private function candidateAmountChecked(ProductIdentificationCandidate $candidate): bool
    {
     ...
     */
    private function candidateAmountMismatch(ProductIdentificationCandidate $candidate): bool
    {
        return $this->candidateAmountChecked($candidate)
            && data_get($candidate->metadata, 'document_line_amount_consistency.is_consistent') === false;
    }
     ...
     */
    private function candidateToTableRow(ProductIdentificationCandidate $candidate): array
    {
